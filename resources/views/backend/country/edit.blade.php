                    ...
                    <div class="card-body">
                        <form method="POST" action="{{ route('back.country.update',['id' =>$info->id]) }}" enctype="multipart/form-data">
                                ...
                                <div class="col-10">
                                    <select class="form-control" name="continent_id" id="continent_id" required>
                                        @foreach($continents as $continent)
                                            <option @if($continent->id == $info->country_id) selected @endif value="{{$continent->id}}">{{$continent->continent_tw}}({{$continent->continent_en}})</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-10">
                                    <input type="file" class="form-control-file" id="country_photo" name="country_photo"  accept="image/*" @if($info->country_photo == null) required @endif>
                                </div>
                                <div class="col-12">
                                    <p class="text-danger">
                                        *注意：1.上傳後，將覆蓋掉原有照片！
